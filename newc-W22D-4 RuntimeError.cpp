...
//        printf("%d %d %d:%f %f\n",l,r,rt,rsin[rt],rcos[rt]);
    ...
    if(tree[rt]){
        ...
        double ss,cc;
        if(from>mid){
            ss=rsin[LL<<1]+rsin[LL<<1|1];cc=rcos[LL<<1]+rcos[LL<<1|1];
            rsin[LL]=ss*cos(tree[LL])+cc*sin(tree[LL]);
            rcos[LL]=cc*cos(tree[LL])-ss*sin(tree[LL]);
        }
        if(to<=mid){
            ss=rsin[RR<<1]+rsin[RR<<1|1];cc=rcos[RR<<1]+rcos[RR<<1|1];
            rsin[RR]=ss*cos(tree[RR])+cc*sin(tree[RR]);
            rcos[RR]=cc*cos(tree[RR])-ss*sin(tree[RR]);
        }
        tree[rt]=0;
    }
