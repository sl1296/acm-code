...
inline bool PolyCut(const vector<Point>& f,const Line& l,vector<Point>& r1,vector<Point>& r2){
    vector<node> ve;
    int n=f.size();
    for(int i=0;i<n;i++){
        Line now=Line(f[i],f[(i+1)%n]);
        if(!isLine(l.v,now.v)&&intersect_in(l,now)){
            ve.push_back((node){LineInter(l,now),i});
        }
    }
...
//                for(int jj=0;jj<poly[i].size();jj++){
