...
void pushdown2(int l,int r,int mid,int rt){
  //  printf("down %d %d val=%d\n",l,r,tree[rt].val);
    int left=rt<<1,right=rt<<1|1;
    tree[left]=tree[right]=tree[rt];
    if(tree[rt].val){
        tree[left].llenb=tree[left].rlenb=tree[left].lenb=tree[left].cnt=mid-l+1;
        tree[right].llenb=tree[right].rlenb=tree[right].lenb=tree[right].cnt=r-mid;
    }else{
        tree[left].llena=tree[left].rlena=tree[left].lena=mid-l+1;
   ...
   // printf("%d %d %d %d %d\n",from,to,l,r,rt);
    if(from<=l&&to>=r){
      //  tree[rt].bj^=1;
       // printf("bj=%d\n",tree[rt].bj);
        qf(rt,r-l+1);
      //  printf("%d %d val=%d bj=%d %d\n",l,r,tree[rt].val,tree[rt].bj,0^1);
        return;
    }
    int mid=(l+r)>>1;
    if(tree[rt].bj)pushdown(l,r,mid,rt);
