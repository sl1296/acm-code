...
int cal(int ss,int ee){
   // printf("ss=%d ee=%d\n",ss,ee);
    while(in[ss]=='('&&in[ee-1]==')'&&pre[ee-1]==ss&&ss<ee-1){
        ss++;ee--;
    }
    if(ss>=ee)return 0;
  /*  for(int i=ss;i<ee;i++){
    ...
    while(~scanf("%s",in)){
        if(strcmp(in,"print")==0){
            ...
            if(pd){
                id1=mi[now];
                if(id1==0){
                    var[cnt]=(node){true,cnt,now,"",false};
                    mi[now]=cnt;
                    id1=cnt;
                    cnt++;
                }
                ...
                id1=ms[in];
                if(id1==0){
                    var[cnt]=(node){false,cnt,-1,in,false};
                    ms[in]=cnt;
                    id1=cnt;
                    cnt++;
                }
          ...
          //  printf("cnt=%d\n",cnt);
            ...
            if(pd){
              ...
              //  printf("id2=%d cnt=%d\n",id2,cnt);
                if(id2==0){
                    var[cnt]=(node){false,cnt,-1,in2,false};
                    ms[in2]=cnt;
                    id2=cnt;
                    cnt++;
                }
          ...
          //  printf("id1=%d id2=%d\n",id1,id2);
            int xx=ffind(id1);
            int yy=ffind(id2);
            if(var[id1].isdef==false&&(xx!=yy||id1==id2)){
            	var[id1].fa=yy;
            	var[id1].isdef=true;
            }
        }
