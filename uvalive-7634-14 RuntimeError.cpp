...
ll sum0[100010];
ll sum1[100010];
void cal(char* ss,int g,int left,int right,ll& r0,ll& r1,bool ad){
//    printf("%d %d %s %d\n",left,right,s,g);
    if(g==0){
        if(ad){
    ...
    if(ss[0]=='D'){
        cal(&ss[1],g-1,left,left^right,r0,r1,ad);
    }else{
...
//        printf("R %d\n",id);
//        if(g-1<0||g-1>100010)while(1);
        r0=(r0+cnt0[id][g-1])%mod;
        r1=(r1+cnt1[id][g-1])%mod;
//        printf("%lld %lld\n",r0,r1);
        cal(&ss[1],g-1,add,right,r0,r1,ad);
    }
}
int main(){
    int cas=1;
...
//    printf("\n");
    ...
    for(int z=1;z<=t;z++){
        ...
        if(g==0){
            ...
            int re1=0,re2=0;
            for(int i=l;i<=r;i++){
                if(s[i]=='0')re1++;else re2++;
            }
            printf("Case %d: %d %d\n",cas++,re1,re2);
            continue;
        }
        scanf("%d%s%d%s",&l,xl,&r,xr);
        l++;r++;
//        if(r-1<0||r-1>100010)while(1);
//        if(l-1<0||l-1>100010)while(1);
        g--;
