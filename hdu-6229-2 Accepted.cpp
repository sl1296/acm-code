...
//                        printf("fz=%d\n",fz);
                        fz+=lx*(1+lx)/2*5-((i==0 || (i>0 && !vis[i-1][j]))?1:0)-((j==0 || (j>0 && !vis[i][j-1]))?lx:0)
                            -((i==mc-1 || (i<mc-1 && !vis[i+1][j]))?lx:0)-((j==mc-1 || (j<mc-1 && !vis[i][j+1]))?1:0);
//                        printf("%d %d %d %d\n",((i==0 || (i>0 && !vis[i-1][j]))?1:0),((j==0 || (j>0 && !vis[i][j-1]))?lx:0)
//                            ,((i==mc-1 || (i<mc-1 && !vis[i+1][j]))?lx:0),((j==mc-1 || (j<mc-1 && !vis[i][j+1]))?1:0));
//                        printf("i=%d j=%d fz=%d\n",i,j,fz);
                    }
                }
