...
void pushdown2(int rt){
    if(tree[rt<<1]==-1)bj[rt<<1]=!bj[rt<<1];else tree[rt<<1]=!tree[rt<<1];
    if(tree[rt<<1|1]==-1)bj[rt<<1|1]=!bj[rt<<1|1];else tree[rt<<1|1]=!tree[rt<<1|1];
    bj[rt]=false;
}
...
void update2(int from,int to,int l,int r,int rt){
    if(from<=l&&to>=r){
        if(tree[rt]==-1)bj[rt]=!bj[rt];else tree[rt]=!tree[rt];
        return;
    }
    int mid=(l+r)>>1;
