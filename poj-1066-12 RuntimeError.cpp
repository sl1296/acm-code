...
inline bool opposite_side_in(const Point& p1,const Point& p2,const Line& l){
    return Cross(l.p-l.b,p1-l.b)*Cross(l.p-l.b,p2-l.b)<=eps;
}
...
inline int PolyCut(const Polygon& f,const Line& l,Polygon& r1,Polygon& r2){
    int n=f.size();
    Point crs[10];
    int crsn[10],cnt=0;
    for(int i=0;i<n;i++){
        Line now=Line(f[i],f[(i+1)%n]);
        if(isLine(now.v,l.v))return -1;
        if(intersect_in(now,l)&&!isLine(now.v,l.v)){ //Èç¹ûÊÇsegÇÐÅÐ¶Ïintersect_in(now,l)
            crs[cnt]=LineInter(now,l);
            crsn[cnt++]=i;
            if(cnt>1&&crs[cnt-2]==crs[cnt-1])cnt--;
        }
    }
    if(cnt<2)return 0;
    r1.push_back(crs[1]);
    r1.push_back(crs[0]);
    for(int i=crsn[0]+1;i<=crsn[1];i++){
        if(!(f[i]==r1[r1.size()-1]))r1.push_back(f[i]);
    }
    if(r1[0]==r1[r1.size()-1])r1.pop_back();
    r2.push_back(crs[0]);
    r2.push_back(crs[1]);
    for(int i=crsn[1]+1;;i++){
        if(i==n)i=0;
        if(!(f[i]==r2[r2.size()-1]))r2.push_back(f[i]);
        if(i==crsn[0])break;
    }
    if(r2[0]==r2[r2.size()-1])r2.pop_back();
    return 1;
}
int main(){
    int n;
