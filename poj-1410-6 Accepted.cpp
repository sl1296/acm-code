...
int main(){
    ...
    while(n--){
        ...
        if(dcmp(a-min(e,g))>=0&&dcmp(max(e,g)-a)>=0&&dcmp(b-min(f,h))>=0&&dcmp(max(f,h)-b)>=0)ans=true;
        if(dcmp(c-min(e,g))>=0&&dcmp(max(e,g)-c)>=0&&dcmp(d-min(f,h))>=0&&dcmp(max(f,h)-d)>=0)ans=true;
        if(ans){
            printf("T\n");
        }else{
            printf("F\n");
