...
struct mp{
    int a,b,c,d;
    bool operator < (const mp &p) const{
...
int main(){
       ...
       //     printf("i=%d sz=%d\n",i,sz);
            ...
            /*    printf("j=%d\n",j);
                for(int jj=0;jj<poly[j].size();jj++){
                    printf("(%f,%f)",poly[j][jj].x,poly[j][jj].y);
                }
                printf("\n");*/
                vector<node> ve;
    ...
    //    printf("siz=%d\n",siz);
      ...
      //      printf("i=%d\n",i);
            ...
            bool ff=false;
            for(int j=0;j<sz1;j++){
                if((dcmp(poly[i][j].x-100)==0&&dcmp(poly[i][(j+1)%sz1].x-100)==0)||(dcmp(poly[i][j].y-100)==0&&dcmp(poly[i][(j+1)%sz1].y-100)==0)){
                    ff=true;
                    break;
                }
            }
