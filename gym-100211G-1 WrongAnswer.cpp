
#include<cstdio>
#include<cstring>
#include<algorithm>
#define ll long long
using namespace std;
struct node{
    ll x,y;
};
node tr[1000010];
ll ys[2000010];
ll ysc;
ll cmp1(node a,node b){
    if(a.x!=b.x)return a.x<b.x;
    return a.y<b.y;
}
ll cmp2(node a,node b){
    if(a.x!=b.x)return a.x>b.x;
    return a.y<b.y;
}
ll cmp3(node a,node b){
    if(a.x!=b.x)return a.x<b.x;
    return a.y>b.y;
}
ll cmp4(node a,node b){
    if(a.x!=b.x)return a.x>b.x;
    return a.y>b.y;
}
ll sum[2000010];
ll len;
ll lowbit(ll x){
    return x&(-x);
}
void add(ll x){
    for(ll i=x;i<=len;i+=lowbit(i)){
        sum[i]++;
    }
}
ll query(ll x){
    ll ret=0;
    for(ll i=x;i>0;i-=lowbit(i)){
        ret+=sum[i];
    }
    return ret;
}
int main(){
    ll m,n;
    while(~scanf("%I64d%I64d",&m,&n)){
        ysc=0;
        for(ll i=0;i<n;i++){
            scanf("%I64d%I64d",&tr[i].x,&tr[i].y);
            ys[ysc++]=tr[i].x;
            ys[ysc++]=tr[i].y;
        }
        sort(ys,ys+ysc);
        len=unique(ys,ys+ysc)-ys;
        for(ll i=0;i<n;i++){
            tr[i].x=lower_bound(ys,ys+len,tr[i].x)-ys+1;
            tr[i].y=lower_bound(ys,ys+len,tr[i].y)-ys+1;
        }
       // printf("len=%lld\n",len);
        ll ans=2e18;
        sort(tr,tr+n,cmp1);
        memset(sum,0,sizeof(sum));
        for(ll i=0;i<n;i++){
            add(tr[i].y);
            ll re=query(tr[i].y);
            if(re*2==n){
                ans=min(ans,(ys[tr[i].x-1]+1)*(ys[tr[i].y-1]+1));
           //     printf("0:%lld %lld %lld\n",ys[tr[i].x-1],ys[tr[i].y-1],ans);
            }
        }
        sort(tr,tr+n,cmp2);
        memset(sum,0,sizeof(sum));
        for(ll i=0;i<n;i++){
            add(tr[i].y);
            ll re=query(tr[i].y);
            if(re*2==n){
                ans=min(ans,(m-ys[tr[i].x-1])*(ys[tr[i].y-1]+1));
           //     printf("1:%lld %lld %lld\n",ys[tr[i].x-1],ys[tr[i].y-1],ans);
            }
        }
        sort(tr,tr+n,cmp3);
        memset(sum,0,sizeof(sum));
        for(ll i=0;i<n;i++){
            add(tr[i].y);
            ll re=query(tr[i].y);
            if(re*2==n){
                ans=min(ans,(ys[tr[i].x-1]+1)*(m-ys[tr[i].y-1]));
               // printf("2:%lld %lld %lld\n",ys[tr[i].x-1],ys[tr[i].y-1],ans);
            }
        }
        sort(tr,tr+n,cmp4);
        memset(sum,0,sizeof(sum));
        for(ll i=0;i<n;i++){
            add(tr[i].y);
            ll re=query(tr[i].y);
            if(re*2==n){
                ans=min(ans,(m-ys[tr[i].x-1])*(m-ys[tr[i].y-1]));
            //    printf("3:%lld %lld %lld\n",ys[tr[i].x-1],ys[tr[i].y-1],ans);
            }
        }
        if(ans==2e18)ans=-1;
        printf("%I64d\n",ans);
    }
    return 0;
}


