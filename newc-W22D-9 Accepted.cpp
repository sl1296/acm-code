...
//        printf("%d %d %d:%f %f\n",l,r,rt,rsin[rt],rcos[rt]);
    ...
    int mid=(l+r)>>1,LL=rt<<1,RR=rt<<1|1;
//    printf("%f %f %f %f\n",rsin[LL],rsin[RR],rcos[LL],rcos[RR]);
    if(tree[rt]){
        tree[LL]+=tree[rt];
        tree[RR]+=tree[rt];
        double ss,cc;
...
//    printf("%d %d %d %d %d %f %f\n",l,r,rt,from,to,rsin[rt],rcos[rt]);
    if(from<=l&&r<=to){
        if(cal){
            if(l==r){
                rsin[rt]=sin(tree[rt]);
...
//    printf("rt=%d %d %d ret1=%f %d %d\n",rt,l,r,ret,l,mid);
    if(to>mid)ret+=query(rson,from,to,cal);
//    printf("rt=%d %d %d ret2=%f %d %d\n",rt,l,r,ret,mid+1,r);
    rsin[rt]=rsin[LL]+rsin[RR];
    rcos[rt]=rcos[LL]+rcos[RR];
    return ret;
}
