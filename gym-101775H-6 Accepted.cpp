...
//        }
        ...
        if(fdn==1 && type[0]==0){
            if(x==1 && y==1)printf("Case #%d: DISLIKE\n",z);
            else printf("Case #%d: SURPRISE\n",z);
            continue;
        }
        bool add=false;
        for(int i=0;i<fdn;++i){
            if(type[i]==0){
                if(i==0||i==fdn-1)type[i]=-1;
                    ...
                    if(le[i]==2){
                        if(type[i-1]==1 && y<=2){
                            if(y==1)type[i]=1;
//                            if(le[i-1]<le[i+1])++le[i-1];
//                            else ++le[i+1];
//                            le[i]=1;
//                            type[i]=2;
                        }else if(type[i-1]==2 && x<=2){
                            if(x==1)type[i]=2;
//                            if(le[i-1]<le[i+1])++le[i-1];
//                            else ++le[i+1];
//                            le[i]=1;
